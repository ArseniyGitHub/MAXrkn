...
void WsSession::on_read(beast::error_code err, size_t received) {
	...
	try {
		...
		pqxx::work w(*con);
		auto res = w.exec_params("(WITH msg AS (INSERT INTO messages (sender_id, message_text, send_time) values ($1, $2, NOW()) RETURNING id, sender_id, send_time) SELECT i.id, i.send_time, u.username FROM msg i JOIN users u ON u.id = i.sender_id)", m["user_id"].get<long long>(), m["content"].get<std::string>());
		w.commit();
		if (!res.empty()) {
			json broadmsg;
